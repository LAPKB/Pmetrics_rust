...
`PM_data$new()`. When combined using `PM_fit$new()` and the `$run()`
function on the resulting object, the analysis is executed. At the end
of the run, the hard drive will contain a new numerically named folder,
e.g., 1, 2, 3, …, that contains the files which can be loaded into R
subsequently using `PM_load(x)`, replacing `x` with the folder number.
...
`PM_model$update()`, `PM_fit$new()` is called and then the run is
executed with `$run()`. If continuing a previous run that did not end,
simply use the `$run()` and specify the run number you wish to continue
as the `prior` argument to `$run()`. These scenarios are illustrated
below.
...
The general Pmetrics workflow in Legacy for IT2B and NPAG is shown in
the following diagram. The major differences compared to R6 are:

- *Data and model* The data is always a .csv file, and the model is
  always a text file which must be edited to make changes. Both must be
  copied from a prior folder to the current working directory for every
  run.
- *Run.* Separate commands for NPAG and IT2B must be used,
  i.e. [`NPrun()`](https://lapkb.github.io/Pmetrics_rust/reference/NPrun.md)
  and `ITrun()`.
- *Loading.* Results are loaded with
  [`PMload()`](https://lapkb.github.io/Pmetrics_rust/reference/PMload.md)
  not
  [`PM_load()`](https://lapkb.github.io/Pmetrics_rust/reference/PM_load.md)
  because the former returns a collection of results all suffixed with
  the run number, e.g. `op.1` or `final.1` for run 1. This can lead to
  confusion if working on two projects at once, or uncertainty about
  what is actually loaded.

![](Images/Slide2.png)

The user supplies the names of the data and model files as arguments to
the run functions. R is used to specify the working directory containing
these files. The analysis is executed in a terminal window by calling
[`NPrun()`](https://lapkb.github.io/Pmetrics_rust/reference/NPrun.md),
`ITrun()` or `ERRrun()` functions. The results can be loaded into R
after the run completes using
[`PMload()`](https://lapkb.github.io/Pmetrics_rust/reference/PMload.md).
The analgous Legacy code to the R6 code above is shown here.
...
While there are fewer lines of R code than for R6, the edits made to

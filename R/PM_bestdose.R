...
PM_bestdose <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(prior,
      ...
      # Parse settings
      if (is.null(settings)) {
        settings <- private$.default_bestdose_settings(prior)
      }
  ...
  private = list(
    ...
    .parse_model = function(model) {
      if (inherits(model, "PM_model")) {
        ...
        compiled_path <- model$model_list$compiled_location
    ...
    .default_bestdose_settings = function(prior) {
      ...
      if (inherits(prior, "PM_result")) {
        ...
        settings <- list(
          algorithm = "NPAG",
          ranges = list(),  # Will be filled from prior
          error_models = list(
            list(
              initial = 0.0,
              type = "additive",
